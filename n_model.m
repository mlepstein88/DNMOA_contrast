...
for t = p.dt:p.dt:p.T
    ...
    % Excitatory drive - edited mle to allow separate contrasts
    if any(inp(p.stimseq(1)))
        drive = halfExp(p.rfresp(logical(inp),:)*p.contrast(1),p.p)'; % select pre-calculated response
    elseif any(inp(p.stimseq(2)))
        drive = halfExp(p.rfresp(logical(inp),:)*p.contrast(2),p.p)'; % select pre-calculated response
    else
